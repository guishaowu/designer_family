...
# Do some performance related information gathering for designer_family.
EXPLANATION="
...
microversion of designer_family followed by an encoded representation of
...
to find the relevant entry in logs/designer_family-api.log.
...
function time_candidates {
    (
        ...
        time curl -s -H 'x-auth-token: admin' -H 'openstack-api-version: designer_family latest' "${PLACEMENT_URL}/allocation_candidates?${PLACEMENT_QUERY}" > /dev/null
        time curl -s -H 'x-auth-token: admin' -H 'openstack-api-version: designer_family latest' "${PLACEMENT_URL}/allocation_candidates?${PLACEMENT_QUERY}" > /dev/null
    ) 2>&1 | tee -a $LOG
}
...
function ab_bench {
    (
        ...
        ab -c $AB_CONCURRENT -n $AB_COUNT -H 'x-auth-token: admin' -H 'openstack-api-version: designer_family latest' "${PLACEMENT_URL}/allocation_candidates?${PLACEMENT_QUERY}"
    ) 2>&1 | tee -a $LOG
}

function write_allocation {
    # Take the first allocation request and send it back as a well-formed allocation
    curl -s -H 'x-auth-token: admin' -H 'openstack-api-version: designer_family latest' "${PLACEMENT_URL}/allocation_candidates?${PLACEMENT_QUERY}&limit=5" \
        | jq --arg proj $(uuidgen) --arg user $(uuidgen) '.allocation_requests[0] + {consumer_generation: null, project_id: $proj, user_id: $user}' \
        | curl -s -H 'x-auth-token: admin' -H 'content-type: application/json' -H 'openstack-api-version: designer_family latest' \
    ...
    if [[ $rp_count -ge $COUNT ]]; then
      ...
      ab_bench
        ...
        (
            ...
            echo "See job-output.txt.gz and logs/designer_family-api.log for additional detail."
        ) | tee -a $LOG
        code=1
    fi
    set -x
