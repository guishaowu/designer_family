...
sudo mysql -uroot -psecret -e "DROP DATABASE IF EXISTS designer_family;"
sudo mysql -uroot -psecret -e "CREATE DATABASE designer_family CHARACTER SET utf8;"
sudo mysql -uroot -psecret -e "GRANT ALL PRIVILEGES ON designer_family.* TO 'root'@'%' identified by 'secret';"

# Create a virtualenv for designer_family to run in
python -m virtualenv -p python3 .designer_faimily
. .designer_faimily/bin/activate
