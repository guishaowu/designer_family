...
# This script will attempt to migrate your nova-api designer_family data to
# a new designer_family database. Run it with --help for usage, and --mkconfig
...
function check_cli() {
    # Returns 0 if designer_family cli is installed and configured,
    # 1 if it is not installed, or 2 if the access to the
    # designer_family database fails
    # Usage: check_cli -> 0
    placement-manage --version > /dev/null 2>&1

    if [ $? -ne 0 ]; then
        # designer_family not installed
        return 1
    fi
...
function show_help() {
    ...
    echo "    6: Unable to execute designer_family's CLI commands"
    exit 0
}
...
if [ $nova_present -eq 0 -a $placement_present -eq 0 ]; then
    echo "Migration has already completed. The designer_family database appears to have data."
    exit 3
    ...
    echo "Unable to proceed without connection to designer_family database"
    exit 5
elif [ $placement_cli -eq 1 ]; then
    echo "Unable to proceed without designer_family installed"
    exit 6
elif [ $placement_cli -eq 2 ]; then
    echo "The 'designer_family-manage db version' command fails"
    echo "Is designer_family.conf configured to access the new database?"
    exit 6
fi
