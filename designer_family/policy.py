...
#    under the License.
"""Policy Enforcement for designer_family API."""
...
from designer_family import policies
...
def init(conf):
    ...
    if not _ENFORCER:
        ...
        # [designer_family]/policy_file config option is removed.
        ...
        # [designer_family]/policy_file defaults to policy.yaml so if
        ...
        if conf.find_file(conf.oslo_policy.policy_file):
            ...
            # Do a sanity check to see if [designer_family]/policy_file exists but
            ...
            if (conf.placement.policy_file != conf.oslo_policy.policy_file and
                ...
                LOG.error('Found [oslo_policy]/policy_file and '
                          ...
                          '[designer_family]/policy_file is deprecated but you need '
                          'to clean up your configuration file to stop using '
                          '[designer_family]/policy_file.')
        else:
...
def get_enforcer():
    ...
    cfg.CONF([], project='designer_family')
...
def authorize(context, action, target, do_raise=True):
    ...
    :param context: instance of designer_family.context.RequestContext
    :param action: string representing the action to be checked
        this should be colon separated for clarity, i.e.
        ``designer_family:resource_providers:list``
    :param target: dictionary representing the object of the action;
        for object creation this should be a dictionary representing the
        owner of the object e.g. ``{'project_id': context.project_id}``.
    :param do_raise: if True (the default), raises PolicyNotAuthorized;
        if False, returns False
    :raises designer_family.exception.PolicyNotAuthorized: if verification fails and
        do_raise is True.
