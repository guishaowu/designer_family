...
from designer_family.db.sqlalchemy import models
from designer_family import db_api


class Checks(upgradecheck.UpgradeCommands):
    """Checks for the ``designer_family-status upgrade check`` command.
    ...
    def _check_root_provider_ids(self):
        ...
        "designer_family-manage db online_data_migrations" command is run during
        ...
        if self._check_missing_root_ids(self.ctxt):
            return upgradecheck.Result(
                ...
                details='There is at least one resource provider table '
                        ...
                        'Run the "designer_family-manage db '
    ...
    def _check_incomplete_consumers(self):
        ...
        be part of deploying/upgrading designer_family automation. However, with
        designer_family being freshly extracted from nova, this check serves as a
        friendly reminder and because the data migration will eventually be
        removed from nova along with the rest of the designer_family code.
        """
        ...
        if missing_consumer_count:
            ...
            return upgradecheck.Result(
                ...
                details='There are %s incomplete consumers table records '
                        ...
                        '"designer_family-manage db online_data_migrations" '
