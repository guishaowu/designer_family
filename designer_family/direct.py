...
#    under the License.
"""Call any URI in the designer_family service directly without real HTTP.
...
class PlacementDirect(interceptor.RequestsInterceptor):
    """Provide access to the designer_family service without real HTTP.

    wsgi-intercept is used to provide a keystoneauth1 Adapter that has access
    to an in-process designer_family service. This provides access to making changes
    to the designer_family database without requiring HTTP over the network - it
    remains in-process.
    ...
    :param conf: An oslo config with the options used to configure
                 the designer_family service (notably database connection
                 string).
    ...
    def __init__(self, conf, latest_microversion=False):
        ...
        if latest_microversion:
            headers['OpenStack-API-Version'] = 'designer_family latest'
        self.adapter = adapter.Adapter(
            session.Session(auth=None, session=request_session,
                            additional_headers=headers),
            service_type='designer_family', raise_exc=False)
