...
class TestTraitHandler(base.ContextTestCase):
    ...
    @mock.patch('designer_family.objects.trait.Trait.create')
    @mock.patch('designer_family.objects.trait.Trait.get_by_name')
    @mock.patch('designer_family.context.RequestContext.can')
    @mock.patch('designer_family.util.wsgi_path_item', return_value='CUSTOM_FOOBAR')
    def test_trait_create_ordering(
        ...
        req.environ['designer_family.context'] = fake_context
        ...
        req.environ['designer_family.microversion'] = microversion
