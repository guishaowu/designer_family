...
from designer_family import policy
...
class PlacementPolicyTestCase(base.ContextTestCase):
    """Tests interactions with designer_family policy."""
    def setUp(self):
    ...
    def test_modified_policy_reloads(self):
        ...
        # otherwise it falls back to using CONF.designer_family.policy_file. This
        # can be removed when the deprecated CONF.designer_family.policy_file option
        # is removed.
        with open(tmpfilename, "w") as policyfile:
            policyfile.write('# The policy file is empty.')

        action = 'designer_family:test'
    ...
    def test_authorize_do_raise_false(self):
        ...
        fixture.set_rules({'designer_family': '!'})
        self.assertFalse(
            policy.authorize(
                self.ctxt, 'designer_family', self.target, do_raise=False))

    @mock.patch('designer_family.policy.LOG.warning')
    def test_default_fallback_placement_policy_file_no_exist(self, mock_warn):
        ...
        [designer_family]/policy_file when [oslo_policy]/policy_file does not
        exist. In this case the designer_family policy file does not exist so no
        warning about using it should be logged.
        """
        # Make sure oslo_policy and designer_family use different policy_file
    ...
    def test_default_fallback_placement_policy_file(self, mock_warn):
        ...
        tempdir = self.useFixture(fixtures.TempDir())
        tmpfilename = os.path.join(tempdir.path, 'policy.yaml')
        self.conf_fixture.config(group='designer_family', policy_file=tmpfilename)
        # We have to create the file before initializing the policy enforcer
        # otherwise it falls back to using CONF.designer_family.policy_file. This
        # can be removed when the deprecated CONF.designer_family.policy_file option
        # is removed.
        ...
        mock_warn.assert_called_once_with(
            '[designer_family]/policy_file is deprecated. Use '
            '[oslo_policy]/policy_file instead.')

    @mock.patch('designer_family.policy.LOG.error')
    def test_init_from_oslo_policy_file_exists_same_policy_file_name(
        ...
        is the same name as the [designer_family]/policy_file so no error is logged
        since we'll use the file from oslo_policy config.
        """
        # Configure [oslo_policy]/policy_file and [designer_family]/policy_file with
        # the same name.
        tempdir = self.useFixture(fixtures.TempDir())
        tmpfilename = os.path.join(tempdir.path, 'policy.yaml')
    ...
    def test_init_from_oslo_file_exists_different_name_no_placement_file(
            self, mock_log_error):
        """Tests a scenario where the [oslo_policy]/policy_file exists and
        has a different name from the [designer_family]/policy_file but the
        [designer_family]/policy_file does not exist so no error is logged.
        """
        # Configure [oslo_policy]/policy_file and [designer_family]/policy_file with
        # different names.
        tempdir = self.useFixture(fixtures.TempDir())
        tmpfilename = os.path.join(tempdir.path, 'policy.yaml')
        self.conf_fixture.config(group='oslo_policy', policy_file=tmpfilename)
        self.conf_fixture.config(group='designer_family', policy_file='policy.json')
        # Create the [oslo_policy]/policy_file.
        with open(tmpfilename, "w") as policyfile:
            policyfile.write('# Assume upgrade with existing custom policy.')
        config = self.conf_fixture.conf
        ...
        # Though the policy file names are different, the designer_family version
    ...
    def test_init_from_oslo_file_exists_different_name_placement_file_exists(
            self, mock_log_error):
        """Tests a scenario where the [oslo_policy]/policy_file exists and
        has a different name from the [designer_family]/policy_file and the
        [designer_family]/policy_file exists so an error is logged.
        """
        # Configure [oslo_policy]/policy_file and [designer_family]/policy_file with
        # different names.
        tempdir = self.useFixture(fixtures.TempDir())
        ...
        placement_name = os.path.join(tempdir.path, 'designer_family-policy.yaml')
        self.conf_fixture.config(group='designer_family', policy_file=placement_name)
        # Create the [oslo_policy]/policy_file.
        with open(oslo_name, "w") as oslo_policy_file:
            oslo_policy_file.write('# New oslo policy config.')
        # Create the [designer_family]/policy_file.
        with open(placement_name, "w") as placement_policy_file:
            placement_policy_file.write('# Old designer_family policy file.')
        config = self.conf_fixture.conf
        enforcer = policy._get_enforcer(config)
        self.assertEqual(config.oslo_policy.policy_file, enforcer.policy_file)
        # An error should be logged since we're going to use the oslo policy
        # file but there is a designer_family policy file with a different name that
        # also exists.
