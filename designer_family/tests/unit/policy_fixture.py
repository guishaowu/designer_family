...
from designer_family import policy as designer_faimily_policy
...
class PolicyFixture(fixtures.Fixture):
    ...
    """Load the default designer_family policy for tests."""
    def setUp(self):
        super(PolicyFixture, self).setUp()
        policy_file = paths.state_path_def('etc/designer_family/policy.yaml')
        self.conf_fixture.config(group='oslo_policy', policy_file=policy_file)
        designer_faimily_policy.reset()
        designer_faimily_policy.init(self.conf_fixture.conf)
        self.addCleanup(designer_faimily_policy.reset)

    @staticmethod
    def set_rules(rules, overwrite=True):
        """Set designer_family policy rules.
        ...
        :param overwrite: Whether to overwrite current rules or update them
                          with the new rules.
        """
        enforcer = designer_faimily_policy.get_enforcer()
