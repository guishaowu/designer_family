...
#    under the License.
"""Utility methods for designer_family API."""
...
# microversion
import designer_family.microversion

LOG = logging.getLogger(__name__)

# Error code handling constants
ENV_ERROR_CODE = 'designer_family.error_code'
...
def json_error_formatter(body, status, title, environ):
    ...
    microversion = designer_family.microversion
...
def resource_class_url(environ, resource_class):
    ...
    If SCRIPT_NAME is present, it is the mount point of the designer_family
    WSGI app.
    """
...
def wsgi_path_item(environ, name):
    ...
    # the value as the entire designer_family API has paths that accept no
    # encoded values.
    try:
...
def normalize_member_of_qs_params(req, suffix=''):
    ...
    :raises `webob.exc.HTTPBadRequest` if the val parameter is not in the
            expected format.
    """
    want_version = req.environ[designer_family.microversion.MICROVERSION_ENVIRON]
