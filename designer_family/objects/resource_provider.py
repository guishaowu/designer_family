...
# used over RPC. Remote manipulation is done with the designer_family HTTP
...
from designer_family.objects import inventory as inv_obj
...
def _get_current_inventory_resources(ctx, rp):
    ...
    :param ctx: `designer_family.context.RequestContext` that may be used to grab a
                DB connection.
...
def _delete_inventory_from_provider(ctx, rp, to_delete):
    ...
    :param ctx: `designer_family.context.RequestContext` that contains an oslo_db
                Session
...
def _set_inventory(context, rp, inv_list):
    ...
    :returns: A list of (uuid, class) tuples that have exceeded their
              capacity after this inventory update.
    :raises designer_family.exception.ConcurrentUpdateDetected: if another thread
...
def _add_traits_to_provider(ctx, rp_id, to_add):
    ...
    :param ctx: `designer_family.context.RequestContext` that has an oslo_db Session
...
def get_all_by_filters(context, filters=None):
    ...
    :param context: `designer_family.context.RequestContext` that may be used to
