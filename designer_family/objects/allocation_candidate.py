...
class AllocationCandidates(object):
    ...
    def get_by_requests(cls, context, groups, rqparams, nested_aware=True):
        ...
        providers. If CONF.designer_family.randomize_allocation_candidates (on
        contex.config) is True (default is False) then the order of the
        allocation requests will be randomized.

        :param context: designer_family.context.RequestContext object.
        :param groups: Dict, keyed by suffix, of designer_family.lib.RequestGroup
...
def _build_provider_summaries(context, rw_ctx, root_ids, prov_traits):
    ...
    :param context: designer_family.context.RequestContext object
    :param rw_ctx: designer_family.research_context.RequestWideSearchContext
