...
def ensure_sync(ctx):
    ...
    traits db. Do so by calling _trait_sync. Since the designer_family API server
    could be multi-threaded, lock around testing _TRAITS_SYNCED to avoid
    duplicating work.

    Different designer_family API server processes that talk to the same database
...
def get_traits_by_provider_tree(ctx, root_ids):
    ...
    :param ctx: designer_family.context.RequestContext object
